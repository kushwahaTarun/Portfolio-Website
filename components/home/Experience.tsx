  ...
  return (
    <Section id="experience" className="relative">
      ...
      <div className="relative">
        <div className="paper relative w-full max-w-full overflow-hidden rounded-[28px] border border-foreground/[0.08] bg-white shadow-[0_30px_80px_-30px_rgba(28,24,21,0.18)]">
          ...
          {/* Main grid */}
          <div className="relative grid min-w-0 gap-0 px-4 pb-4 pt-2 md:px-10 md:pb-10 md:pt-4 lg:grid-cols-[260px_minmax(0,1fr)_240px]">
...
function ConsoleHeader() {
  return (
    <div className="relative flex items-center justify-between border-b border-foreground/[0.08] px-4 py-3 md:px-10 md:py-4">
...
function RoleTabs({
  ...
  return (
    <div className="relative min-w-0 border-b border-foreground/[0.08] py-6 md:border-b-0 md:border-r md:pr-6">
      ...
      <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 [-ms-overflow-style:none] [scrollbar-width:none] md:mx-0 md:flex-col md:gap-1.5 md:overflow-visible md:px-0 md:pb-0 [&::-webkit-scrollbar]:hidden">
        {roles.map((r, i) => {
          ...
          return (
            <button
              ...
              onClick={() => onChange(i)}
              className={cn(
                "group relative w-[78%] shrink-0 cursor-pointer rounded-xl border px-3.5 py-3 text-left transition-colors sm:w-[60%] md:w-auto",
                isActive
...
function RoleStage({ role, active }: { role: Role; active: number }) {
  ...
  return (
    <div className="relative min-w-0 px-0 py-6 md:px-10 md:py-2">
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            <span className="inline-flex items-center gap-2 rounded-full border border-foreground/10 bg-white px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-foreground sticker sm:tracking-[0.22em]">
              <span className="size-1.5 rounded-full bg-[--color-brand-orange]" />
              {role.company}
            </span>
            {isLive && (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/[0.08] px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-emerald-700 sm:tracking-[0.22em]">
                <span className="relative flex size-1.5">
                  <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-500 opacity-60" />
                  <span className="relative inline-flex size-1.5 rounded-full bg-emerald-500" />
                </span>
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground sm:tracking-[0.22em]">
              {role.start} → {role.end}
            </span>
          </div>

          <h3 className="mt-5 break-words text-xl font-semibold leading-[1.1] tracking-tight text-foreground sm:text-3xl md:text-[2.6rem]">
            {role.role}
          </h3>

          <div className="mt-3 flex flex-wrap items-baseline gap-x-3 gap-y-2">
            <span
              className="relative inline-block bg-clip-text pr-1 font-display text-4xl italic leading-[1] tracking-tight text-transparent sm:pr-2 sm:text-[3.2rem] md:text-[4.4rem]"
              style={{
                backgroundImage:
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground sm:text-[11px] sm:tracking-[0.24em]">
              ← from {startYear}
            </span>
          </div>

          <p className="mt-5 max-w-2xl text-pretty text-sm leading-relaxed text-muted-foreground sm:text-[15px] md:text-base">
            {role.summary}
          </p>
        </motion.div>
      </AnimatePresence>
...
function RoleStats({
  ...
  return (
    <div className="relative min-w-0 border-t border-foreground/[0.08] py-6 md:border-l md:border-t-0 md:pl-6">
      <div className="mb-4 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">
        <span className="h-px w-4 bg-foreground/15" />
        Stats
      </div>
...
function StatCard({
  ...
  return (
    <div className="group relative min-w-0 overflow-hidden rounded-xl border border-foreground/[0.08] bg-white p-2.5 transition-all hover:border-foreground/15 hover:shadow-[0_8px_24px_-12px_rgba(28,24,21,0.18)] sm:p-3.5">
      <div className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground sm:tracking-[0.24em]">
...
function Counter({ to, resetKey }: { to: number; resetKey: number }) {
  ...
  return (
    <span className="font-display text-xl italic leading-none tabular-nums text-foreground sm:text-3xl">
      {value}
    </span>
  );
}
...
function Highlights({ role, active }: { role: Role; active: number }) {
  return (
    <div className="relative border-t border-foreground/[0.08] px-4 py-5 md:px-10 md:py-8">
      <div className="mb-4 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">
        <span className="h-px w-4 bg-foreground/15" />
        Shipped
